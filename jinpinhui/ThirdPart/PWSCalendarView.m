...
- (void) SetTimeHeadView
{
    ...
    m_time_head_view.backgroundColor = [UIColor clearColor];
    [self addSubview:m_time_head_view];
    
    m_label_time = [[UILabel alloc] init];
    [m_label_time setFrame:m_time_head_view.bounds];
    [m_label_time setText:@"2014-2-29"];
    m_label_time.font = [UIFont systemFontOfSize:14];
    m_label_time.backgroundColor = [UIColor clearColor];
...
//    origin_x += PWSCalendarSeperateLineHeight;
    ...
    for (int i=0; i<7; i++)
    {
        ...
        CGRect each_day_frame = CGRectMake(i*day_width, 20, day_width, 20);
        [each_day setFrame:each_day_frame];
        each_day.font = [UIFont systemFontOfSize:14];
        if (i == 0 || i == 6) {
            each_day.textColor = UIColorFromRGB(160, 162, 163);
        }else{
             each_day.textColor = UIColorFromRGB(0, 121, 246);
        }
...
- (void) SetCollectionView
{
    ...
    [layout setItemSize:CGSizeMake(width, height-[self GetHeaderViewHeight] + 20)];
    [layout setScrollDirection:UICollectionViewScrollDirectionHorizontal];
    
    m_view_calendar = [[UICollectionView alloc] initWithFrame:CGRectMake(0, [self GetHeaderViewHeight], width, height-[self GetHeaderViewHeight] + 20) collectionViewLayout:layout];
...
- (void) PWSCalendar:(PWSCalendarView *)_calendar didChangeViewHeight:(CGFloat)_height
{
    ...
    float calendar_height = height-[self GetHeaderViewHeight] + 20;
